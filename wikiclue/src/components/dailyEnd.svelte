<script lang="ts">
    ...
    let statistics = {'Played' : 24, 'Win %': '85%', 'Current Streak': 3, 'Max Streak': 7};
    let distribution = [4, 1, 3, 3, 4, 1];
    let max = 4;
    const isOverlayOpen = writable(false);
</script>


<Overlay header="Daily Challenge" onClose={() => {isOverlayOpen.set(false); levelOpen = false;}}>
